...
exports.validateKafkaEnv = validateKafkaEnv;
exports.logKafkaConfig = logKafkaConfig;
/**
 * Valida e retorna configurações Kafka do ambiente
 * @throws Error se KAFKA_BROKERS não estiver definida
 */
function validateKafkaEnv() {
    const kafkaBrokers = process.env.KAFKA_BROKERS?.trim();
    if (!kafkaBrokers) {
        throw new Error(`[Kafka Env Validation] KAFKA_BROKERS is required. Format: "localhost:9092" or "broker1:9092,broker2:9092"`);
    }
    // Validar formato dos brokers
    const brokers = kafkaBrokers.split(',').map((b) => b.trim());
    for (const broker of brokers) {
        if (!broker.includes(':')) {
            throw new Error(`[Kafka Env Validation] Invalid broker format: "${broker}". Expected "host:port"`);
        }
        const [host, port] = broker.split(':');
        const portNum = Number.parseInt(port, 10);
        if (Number.isNaN(portNum) || portNum < 1 || portNum > 65535) {
            throw new Error(`[Kafka Env Validation] Invalid port for broker "${broker}": ${port} (expected 1-65535)`);
        }
    }
    return {
