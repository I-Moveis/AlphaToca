        ...
        components: {
            schemas: {
                User: {
                    ...
                    properties: {
                        ...
                        isIdentityVerified: {
                            type: 'boolean',
                            default: false,
                            description: 'LL-017 — flag do "selo verificado" de identidade. Read-only pela API neste epic (sem endpoint PATCH); writable via Prisma Studio/seed enquanto o setter admin está fora de escopo.',
                        },
                        identityVerifiedAt: {
                            type: 'string',
                            format: 'date-time',
                ...
                Contract: {
                    ...
                    description: 'Contrato de locação. Fonte de verdade do "ciclo ativo" (status=ACTIVE ⇒ Property.status=RENTED). O campo `pdfUrl` (antigo `contractUrl`) armazena o PDF atualmente anexado — vazio enquanto não há documento; `signedAt` marca o upload do PDF assinado via PUT /api/contracts/:id/signed-document (US-016).',
                    properties: {
                        id: { type: 'string', format: 'uuid' },
                        propertyId: { type: 'string', format: 'uuid' },
                        tenantId: { type: 'string', format: 'uuid' },
                ...
                RentalPayment: {
                    ...
                    description: 'Registro mensal do status do aluguel de um imóvel, indexado por (propertyId, period). Usado pelos endpoints GET/PUT /api/properties/{id}/payments/current. Quando ainda não há linha para o mês corrente, a API responde com o default AWAITING sem persistir — a gravação ocorre apenas via PUT (upsert).',
                    properties: {
                        id: { type: 'string', format: 'uuid' },
                        propertyId: { type: 'string', format: 'uuid' },
                        period: {
                ...
                Conversation: {
                    ...
                    description: 'Thread de chat canônica entre um (landlord, tenant) em torno de um Property. O `id` é a referência estável usada pelo frontend (substitui os ids sintéticos antigos `property-<pid>-tenant-<tid>`). Resolvida via GET /api/conversations/resolve (US-012), que faz upsert na chave composta (propertyId, landlordId, tenantId). `messages` é sempre `[]` neste PRD — mensagens de chat estão fora do escopo (futura tabela dedicada).',
                    properties: {
                        id: { type: 'string', format: 'uuid' },
                        propertyId: { type: 'string', format: 'uuid' },
                        landlordId: { type: 'string', format: 'uuid' },
                ...
                SupportTicket: {
                    ...
                    description: 'Ticket de suporte aberto por um usuário final (tenant/landlord) ou por um admin. `code` é o protocolo humano SUP-AAMMDD-XXXX gerado no POST (US-018). `userRole` captura o role no momento da abertura — mesmo que o role do usuário mude depois, o valor aqui preserva o contexto. `resolution` é preenchido pelo admin quando o status transiciona para RESOLVED via PUT /api/admin/support/tickets/{id} (US-020).',
                    properties: {
                        id: { type: 'string', format: 'uuid' },
                        code: {
                            type: 'string',
