...
const conversationSocketService_1 = require("../services/conversationSocketService");
const db_1 = __importDefault(require("../config/db"));
const conversationValidation_1 = require("../utils/conversationValidation");
exports.conversationController = {
    ...
    async resolve(req, res, next) {
        try {
            ...
            const { propertyId, tenantId } = conversationValidation_1.resolveConversationQuerySchema.parse(req.query);
            ...
            if (!isLandlord && !isTenant) {
                return res.status(403).json({
                    ...
                    messages: [
                        {
                            message: 'Only the property owner or the specified tenant can resolve this conversation.',
                        },
                    ],
                });
            }
            const conversation = await conversationService_1.conversationService.resolve(propertyId, property.landlordId, tenantId);
    ...
    async list(req, res, next) {
        try {
            ...
            const { unreadOnly } = conversationValidation_1.listConversationsQuerySchema.parse(req.query);
            const summaries = await conversationService_1.conversationService.list(localUser.id, unreadOnly === 'true');
    ...
    async listMessages(req, res, next) {
        try {
            ...
            const { id } = conversationValidation_1.listConversationMessagesParamsSchema.parse(req.params);
            const { before, limit } = conversationValidation_1.listConversationMessagesQuerySchema.parse(req.query);
            ...
            const { messages, markedReadIds } = await conversationService_1.conversationService.listMessages(id, localUser.id, limit, before);
            ...
            if (markedReadIds.length > 0) {
                conversationSocketService_1.conversationSocketService.emitMessagesRead({ id, landlordId: conversation.landlordId, tenantId: conversation.tenantId }, localUser.id, markedReadIds);
            }
    ...
    async createMessage(req, res, next) {
        try {
            ...
            const { id } = conversationValidation_1.createConversationMessageParamsSchema.parse(req.params);
            const { content } = conversationValidation_1.createConversationMessageBodySchema.parse(req.body);
            const conversation = await db_1.default.conversation.findUnique({
                where: { id },
                select: { landlordId: true, tenantId: true },
            });
            ...
            const message = await conversationService_1.conversationService.createMessage(id, localUser.id, localUser.name, content);
            ...
            conversationSocketService_1.conversationSocketService.emitNewMessage({ id, landlordId: conversation.landlordId, tenantId: conversation.tenantId }, message);
    ...
    async markAllRead(req, res, next) {
        try {
            ...
            const { id } = conversationValidation_1.markConversationReadParamsSchema.parse(req.params);
            const conversation = await db_1.default.conversation.findUnique({
                where: { id },
                select: { landlordId: true, tenantId: true },
            });
            ...
            const updatedIds = await conversationService_1.conversationService.markAllRead(id, localUser.id);
            if (updatedIds.length > 0) {
                conversationSocketService_1.conversationSocketService.emitMessagesRead({ id, landlordId: conversation.landlordId, tenantId: conversation.tenantId }, localUser.id, updatedIds);
            }
