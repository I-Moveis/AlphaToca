...
const supportTicketMessageService_1 = require("../services/supportTicketMessageService");
const supportTicketSocketService_1 = require("../services/supportTicketSocketService");
const supportEmailService_1 = require("../services/supportEmailService");
const supportTicketValidation_1 = require("../utils/supportTicketValidation");
...
function mapAuthorRole(role) {
    switch (role) {
        case 'TENANT':
            return client_1.SupportUserRole.TENANT;
        case 'LANDLORD':
            return client_1.SupportUserRole.LANDLORD;
        case 'ADMIN':
            return client_1.SupportUserRole.ADMIN;
        default:
            ...
            logger_1.logger.warn({ role }, '[supportTicket] unknown author role; defaulting to TENANT');
            return client_1.SupportUserRole.TENANT;
    }
}
exports.supportTicketController = {
    ...
    async create(req, res, next) {
        try {
            ...
            const payload = supportTicketValidation_1.createSupportTicketSchema.parse(req.body);
            const ticket = await supportTicketService_1.supportTicketService.create({
                id: localUser.id,
                name: localUser.name,
            ...
            void supportEmailService_1.supportEmailService.sendTicketCreated(ticket).catch((err) => {
                logger_1.logger.error({ err, ticketId: ticket.id, code: ticket.code }, '[supportTicket] sendTicketCreated threw unexpectedly');
            });
            ...
            if (error instanceof supportTicketService_1.SupportTicketError) {
    ...
    async listForUser(req, res, next) {
        try {
            ...
            const tickets = await supportTicketService_1.supportTicketService.listForUser(localUser.id);
    ...
    async listForAdmin(req, res, next) {
        try {
            const parsed = supportTicketValidation_1.listSupportTicketsQuerySchema.parse(req.query);
            const result = await supportTicketService_1.supportTicketService.list({
    ...
    async updateForAdmin(req, res, next) {
        try {
            const idParsed = zod_1.z.string().uuid().safeParse(req.params.id);
            if (!idParsed.success) {
            ...
            const payload = supportTicketValidation_1.updateSupportTicketSchema.parse(req.body);
            const statusChanging = payload.status !== undefined;
            const view = await supportTicketService_1.supportTicketService.updateForAdmin(idParsed.data, payload);
            ...
            if (statusChanging) {
                ...
                const ticketForEmail = {
                    ...
                    userRole: (view.user?.role ?? 'TENANT'),
                    status: view.status,
                ...
                void supportEmailService_1.supportEmailService.sendTicketUpdated(ticketForEmail).catch((err) => {
                    logger_1.logger.error({ err, ticketId: view.id, code: view.code }, '[supportTicket] sendTicketUpdated threw unexpectedly');
                });
                // Emite mudança de status via WebSocket para o opener e admins
                if (view.user?.id) {
                    supportTicketSocketService_1.supportTicketSocketService.emitTicketUpdated(view.id, view.user.id, {
                        ticketId: view.id,
                        status: view.status,
                        resolution: view.resolution,
                    });
    ...
    async getMessages(req, res, next) {
        try {
            ...
            const ticket = await supportTicketService_1.supportTicketService.findById(ticketId);
            if (!ticket) {
            ...
            const { since } = supportTicketValidation_1.getTicketMessagesQuerySchema.parse(req.query);
            const messages = await supportTicketMessageService_1.supportTicketMessageService.list(ticketId, since ? new Date(since) : undefined);
    ...
    async sendMessage(req, res, next) {
        try {
            ...
            const ticketId = req.params.id;
            const { content, clientMessageId } = supportTicketValidation_1.sendTicketMessageSchema.parse(req.body);
            const message = await supportTicketMessageService_1.supportTicketMessageService.send({
                ticketId,
            ...
            const ticket = await supportTicketService_1.supportTicketService.findById(ticketId);
            if (ticket) {
                supportTicketSocketService_1.supportTicketSocketService.emitTicketMessage(ticketId, ticket.userId, {
                    ticketId,
                    message: {
                        id: message.id,
            ...
            if (error instanceof supportTicketMessageService_1.SupportTicketMessageError) {
                return res.status(error.httpStatus).json({
                    status: error.httpStatus,
                    code: error.code,
                    messages: [{ message: error.message }],
