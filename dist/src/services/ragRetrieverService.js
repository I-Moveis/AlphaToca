...
async function retrieveRelevantChunks(query, overrideDeps) {
    if (typeof query !== "string" || query.trim() === "") {
        return [];
    }
    const deps = overrideDeps ?? getDefaultDeps();
    const k = deps.k ?? rag_1.RETRIEVER_K;
    ...
    const semanticRows = await deps.prisma.$queryRawUnsafe(`SELECT id, content, title, (embedding <=> $1::vector) AS distance
       FROM knowledge_documents
      WHERE embedding IS NOT NULL
      ORDER BY embedding <=> $1::vector ASC
      LIMIT $2`, vectorLiteral, k * 2);
    ...
    if (keywords.length > 0) {
        ...
        try {
            textRows = await deps.prisma.$queryRawUnsafe(`SELECT id, content, title, 0.5::float8 AS distance
           FROM knowledge_documents
          WHERE embedding IS NOT NULL
            AND (${ilikeConditions})
          ORDER BY length(content) ASC
          LIMIT $${keywords.length + 3}`, vectorLiteral, k, ...ilikeParams, k);
        }
        catch {
    ...
    const merged = Array.from(seen.values())
        .map((row) => {
        const distance = typeof row.distance === "string"
...
function extractKeywords(query) {
    ...
    const unique = Array.from(expanded)
        ...
        .filter((kw, i, arr) => !arr.some((other, j) => j < i && other.includes(kw)))
        .slice(0, 6);
    return unique;
}
