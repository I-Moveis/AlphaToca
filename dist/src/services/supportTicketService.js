...
exports.supportTicketService = exports.SupportTicketError = void 0;
...
exports.SupportTicketError = SupportTicketError;
...
function generateTicketCode(now = new Date()) {
...
exports.supportTicketService = {
     ...
     */
    async create(author, payload, now = new Date()) {
        for (let attempt = 0; attempt < CODE_MAX_RETRIES; attempt++) {
            const code = generateTicketCode(now);
            try {
                return await db_1.default.supportTicket.create({
                    data: {
                        code,
                        title: payload.title,
        ...
        throw new SupportTicketError(500, 'CODE_GENERATION_FAILED', `Failed to generate a unique ticket code after ${CODE_MAX_RETRIES} attempts.`);
    },
    /**
    ...
    async list(params) {
        ...
        const data = rows.map((r) => toAdminView(r));
    ...
    async listForUser(userId) {
        return db_1.default.supportTicket.findMany({
            where: { userId },
            orderBy: { createdAt: 'desc' },
            include: {
                messages: {
     ...
     */
    async findById(id) {
        return db_1.default.supportTicket.findUnique({ where: { id } });
    },
    /**
     ...
     */
    async updateForAdmin(id, payload) {
        const existing = await db_1.default.supportTicket.findUnique({ where: { id } });
        if (!existing) {
        ...
        if (payload.assignedToId && payload.assignedToId !== existing.assignedToId) {
            ...
            if (!assignee) {
                throw new SupportTicketError(400, 'ASSIGNEE_NOT_FOUND', `Assignee user ${payload.assignedToId} not found.`);
            }
        }
        const data = {};
